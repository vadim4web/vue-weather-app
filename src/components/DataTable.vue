      ...
      <table @contextmenu.prevent="openMenu">
        ...
        <tr class="navigation clickable">
          <td @click="sortList('city')" class="city cities chosen">
            <span
              v-show="chosenColumn === 'city'"
              :class="[!sortedbyASC ? 'mirrorVertSpan' : 'normalVertSpan']"
              >V</span
            >
            by city
            <span
          ...
          <td @click="sortList('day_min')" class="day_min mins">
            <span
              v-show="chosenColumn === 'day_min'"
              :class="[!sortedbyASC ? 'mirrorVertSpan' : 'normalVertSpan']"
              >V</span
            >
            by min
          ...
          <td @click="sortList('day_max')" class="day_max maxs">
            <span
              v-show="chosenColumn === 'day_max'"
              :class="[!sortedbyASC ? 'mirrorVertSpan' : 'normalVertSpan']"
              >V</span
            >
            by max
        ...
        <tr v-for="(item, index) in collected" :key="index">
          <td @dblclick="removeFromGeo" class="cities removeAble">
            {{ item.city }}
          </td>
          <td @dblclick="removeFromGeo" class="mins removeAble">
            {{ item.day_min }}
          </td>
          <td @dblclick="removeFromGeo" class="maxs removeAble">
            {{ item.day_max }}
          </td>
        </tr>
        <tr>
  ...
  watch: {
    geo: {
      ...
      handler: function (newValue) {
        ...
        const params =
          "&timezone=auto&daily=temperature_2m_max,temperature_2m_min";
  ...
  methods: {
    sortList(sortBy) {
      ...
      if (sortBy === "city") {
        ...
        document
          .getElementsByClassName("day_min")[0]
          .classList.remove("chosen");
        document
          .getElementsByClassName("day_max")[0]
          .classList.remove("chosen");
      }
      if (sortBy === "day_min") {
      ...
      if (this.sortedbyASC) {
        sorted = this.collected.sort((x, y) =>
